...
CREATE TABLE `__new_magic_link_tokens` (
	...
	CONSTRAINT "magic_link_tokens_purpose_chk" CHECK("__new_magic_link_tokens"."purpose" IN ('signup', 'login', 'email_change', 'invite', 'reactivate'))
);
...
CREATE INDEX `idx_magic_link_tokens_member` ON `magic_link_tokens` (`member_id`);