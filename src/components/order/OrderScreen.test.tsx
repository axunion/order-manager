...
describe("OrderScreen", () => {
  it("shows loading state before bootstrap completes", () => {
    // Mock never resolves to simulate loading
    vi.stubGlobal(
      "fetch",
      vi.fn().mockImplementation(() => new Promise(() => {})),
    );
    const { getByText } = render(() => (
      <OrderScreen seatToken="test-token" seatName="テーブル1" />
    ));
  ...
  it("shows error when bootstrap API call fails", async () => {
    ...
    const { findByRole } = render(() => (
      <OrderScreen seatToken="bad-token" seatName="?" />
    ));
  ...
  it("renders the menu after successful bootstrap", async () => {
    ...
    const { findByText } = render(() => (
      <OrderScreen seatToken="test-token" seatName="テーブル2" />
    ));
  ...
  it("shows order items and total when order exists", async () => {
    ...
    const { findByText } = render(() => (
      <OrderScreen seatToken="test-token" seatName="テーブル3" />
    ));
    // "合計" label appears only in the order-total row, not in menu section
    await findByText("合計");
    // The payment button appears when order is open and has items
  ...
  it("clicking '注文する' calls POST /items and updates order", async () => {
    ...
    // After ordering, the order summary shows 合計 (appears only in order-total row)
  ...
  it("'会計をお願いする' button calls PATCH request-payment", async () => {
    const fetchMock = mockFetch([
      ...
      // Bootstrap re-fetch after payment request
      {
        url: "/api/order/",
        method: "GET",
        json: {
